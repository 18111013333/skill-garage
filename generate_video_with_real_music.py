...
BGM_PATH = "/tmp/piano_music.wav"  # 真实钢琴音乐
...
def create_video(voiceover_path):
    ...
    # 验证背景音乐
    audio_array = bgm.to_soundarray()
    sample = audio_array[:int(44100 * 0.5)]
    diff = np.diff(sample[:, 0])
    std_val = np.std(diff)
    print(f"✅ 背景音乐标准差: {std_val:.4f}")
    ...
    # 保存视频
    output_path = os.path.join(OUTPUT_DIR, "视力训练爆款视频_真实音乐.mp4")
