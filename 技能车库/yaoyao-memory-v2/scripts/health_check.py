...
# 使用路径自动发现
from paths import get_memory_base, get_openclaw_home, get_vectors_db, check_sqlite_vec_installed
...
SKILL_CONFIG_DIR = Path(__file__).parent.parent / "config"
...
class HealthChecker:
    ...
    def check_vector_system(self):
        ...
        checks = {
            ...
            "sqlite_vec": vec_installed,
            "cache_size": EMBEDDING_CACHE.stat().st_size if EMBEDDING_CACHE.exists() else 0
        }
        
        # 只要缓存和 chroma_db 存在就算通过
        all_ok = checks["embedding_cache"] and checks["chroma_db"]
    ...
    def check_cache_hit_rate(self):
        ...
        # 估算命中率（基于缓存大小）
        hit_rate = min(100, cache_size * 10)  # 粗略估算
    ...
    def check_mcp_pipeline(self):
        ...
        try:
            ...
            enabled_count = sum(1 for c in connections.values() if c.get('enabled', False))
