...
class UploadFileHwDrive(object):
    ...
    def get_file_list(self, folder_id):
        ...
        while True:
            page_count += 1
            # 构建URL，包含分页参数
            url = f"{base_url}{DRIVE_URL_PATH}?fields=files(fileName),nextCursor&queryParam=parentFolder='{folder_id}'&pageSize=100"
            if next_cursor:
        ...
        logger.info(f"get_file_list total pages: {page_count}, total files count: {len(all_files)}")
        print(f"get_file_list total pages: {page_count}, total files count: {len(all_files)}")
        file_name_list = []
    ...
    def check_file_exists(self, file_name, folder_id):
        ...
        url = f"{base_url}{DRIVE_URL_PATH}?fields=files(id){query_param}fileName='{file_name}'"
        headers = {
            "Authorization": self.auth
        }
        try:
    ...
    def create_file_content(self, file_name, content):
        ...
        try:
            response = requests.post(url, headers=headers, data=content, verify=True, timeout=60)
            if response.status_code == 200:
                result = response.json()
                return result
            else:
    ...
    def update_file_resume(self, file_name, file_id, file_size, parent_folder_id):
        ...
        try:
            response = requests.patch(url, headers=headers, json=data, verify=True, timeout=60)
            if response.status_code == 200:
                result = response.json()
                headers = response.headers
                logger.info(f"headers = {headers}")
...
def query_file_exists(auth, file_name):
    ...
    if parent_folder_id == "":
        # 文件夹不存在，文件也不存在
        logger.error("the folder celiaclaw does not exist in cloud drive")
        return 1
    exists = hw_drive.check_file_exists(file_name, parent_folder_id)
...
def query_folder_exists(auth, folder_name):
    ...
    """
    hw_drive = UploadFileHwDrive(auth)
    exists = hw_drive.check_folder_exists(folder_name)
    if exists == "TOKEN_EXPIRED":
        print_token_expired()
        return -1
    code = int(not exists)
    return code
...
def upload_file_to_celiaclaw(auth, file_path, mode):
    ...
    if auth == "":
        ...
        return
    if not os.path.exists(file_path):
        set_upload_result(result, "error", f"file does not exist: {file_path}")
        return
    hw_drive = UploadFileHwDrive(auth)
    parent_folder_id = hw_drive.get_celiaclaw_file_id()
    ...
    if parent_folder_id == "":
        set_upload_result(result, "error", "create parent folder failed")
        return
    ...
    try:
        ...
        if space_check == "TOKEN_EXPIRED":
            ...
            return -1
        if not space_check:
            set_upload_result(result, "error", "not enough cloud space")
            return
        ...
        if not create_resume_result:
            set_upload_result(result, "error", "create cloud file failed")
            return
        ...
        if not resume_result:
            set_upload_result(result, "error", "upload file failed")
            return
        result["file_id"] = resume_result
        set_upload_result(result, "success", "upload file success")
        return
...
def parse_arguments():
    ...
    parser.add_argument("--id", type=str, help="查询ID，用于query命令")
