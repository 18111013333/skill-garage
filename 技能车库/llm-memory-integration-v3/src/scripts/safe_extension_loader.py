...
"""
安全扩展加载模块 v3.0
增强版：添加加密验证、签名检查、来源追溯、信任列表保护

⚠️ 安全警告：
- 本模块加载原生 SQLite 扩展，存在远程代码执行风险
- 信任列表文件受权限保护，仅允许所有者读写
- 任何对信任列表的修改都会被记录和审计
"""
...
# 审计日志文件
AUDIT_LOG_FILE = Path.home() / ".openclaw" / "extensions" / ".extension_audit.log"
...
def _verify_trusted_hashes_permissions() -> Tuple[bool, str]:
    """
    验证信任列表文件权限
    ...
    Returns:
        (是否安全, 原因)
    """
    if not TRUSTED_HASHES_FILE.exists():
        return True, "文件不存在"
    
    try:
        file_stat = TRUSTED_HASHES_FILE.stat()
        mode = oct(file_stat.st_mode)[-3:]
        
        # 检查权限：只允许所有者读写 (600 或 400)
        if mode not in ['600', '400']:
            # 尝试修复权限
            os.chmod(TRUSTED_HASHES_FILE, 0o600)
            _audit_log("PERMISSION_FIX", f"修复信任列表权限: {mode} -> 600")
            return True, f"已修复权限: {mode} -> 600"
        
        return True, f"权限正确: {mode}"
    except Exception as e:
...
def load_trusted_hashes() -> Dict[str, dict]:
    """加载信任的哈希数据库"""
    # 验证权限
    is_safe, reason = _verify_trusted_hashes_permissions()
    if not is_safe:
        _audit_log("PERMISSION_ERROR", reason)
        return {}
    ...
    if TRUSTED_HASHES_FILE.exists():
        try:
            with open(TRUSTED_HASHES_FILE, 'r') as f:
                data = json.load(f)
                _audit_log("LOAD", f"加载 {len(data)} 个信任哈希")
                return data
        except Exception as e:
            _audit_log("LOAD_ERROR", str(e))
            return {}
    return {}
...
def save_trusted_hashes(hashes: Dict[str, dict]):
    ...
    # 记录审计日志
    _audit_log("SAVE", f"保存 {len(hashes)} 个信任哈希")
