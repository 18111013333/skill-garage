...
description: LLM Memory Integration v5.2.23 - 生产就绪版本。新增华为鲲鹏/海思 ARM64 优化支持，自动检测 NEON/SVE/SVE2 指令集，支持 KML 数学库和毕昇编译器。新增 Intel FMA 加速支持。新增自动化测试套件和 CI/CD 配置。新增系统级性能优化（NUMA 亲和性、大页内存、CAS、IRQ 隔离）。移除内置 SQLite 依赖，用户可自行选择最优实现。所有自动功能默认禁用，资源泄漏已修复，Web API 已删除，HTTP 监控已删除。包含完整安装规范和高风险能力声明。
version: 5.2.23
license: MIT-0
author: xkzs2007
